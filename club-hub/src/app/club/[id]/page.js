...
async function getClubData(name) {
  // NOTE: In production, change 'http://localhost:3000' to your real domain
...
export default async function ClubDetails({ params }) {
  // Wait for params to be ready (required in newer Next.js versions)
  const { id } = await params;

  // Fetch data from YOUR API
  const club = await getClubData(id);

  // If API returns nothing, show 404
  if (!club) {
    return notFound();
  }
        ...
        <div className="max-w-6xl mx-auto px-6 relative -mt-20 md:-mt-24 mb-12">
            <div className="flex flex-col md:flex-row items-end md:items-center gap-6">
                ...
                <div className="flex-grow pb-2 md:pb-8 text-center md:text-left">
                    ...
                    {/* Display Vibe if available, otherwise generic text */}
                    <p className="text-xl font-bold text-club-blue opacity-90">
                        {club.vibe || "Student Community"}
                    </p>
                </div>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-3">
                ...
                <SocialLink 
                    ...
                    link={club.link}
                    icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>}
                />
            </div>
        </div>
